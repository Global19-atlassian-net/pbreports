...
from .models import SymbolTypes, REGISTERED_FILE_TYPES
...
def resolve_tool_contract(tool_contract, input_files, root_output_dir, root_tmp_dir, max_nproc, tool_options):
    ...
    if len(input_files) != len(tool_contract.task.input_file_types):
        raise ToolContractError("Incompatible input types. Supplied {i}. Expected file types {t}".format(i=input_files, t=tool_contract.task.input_file_types))

    # Need to clarify how FileTypes defined in the globaly registry are required,
    # or not required.
